...
const Cartpage = () => {
  ...
  useEffect(() => {
    if (!Auth.isLogin()) navigate("/signin");
  }, []);
  ...
  useEffect(() => {
    const newTotalPrice = allProducts.reduce((total, product) => {
      const _product = userProducts.find(
        (item) => item.productId === product._id
      );
      return total + product.price * _product.quantity;
    }, 0);
  ...
  return (
    <div className="cart-page flex-row gap-sm align-center">
        ...
        <div className="billing-container flex-col gap-sm align-left">
          ...
          <div className="total-price flex-row max-wdth">
            ...
            <div className="total-price-value">
              <p className="body-bld capitalize">{dongFormatter(totalPrice)}</p>
            </div>
          </div>
        </div>
